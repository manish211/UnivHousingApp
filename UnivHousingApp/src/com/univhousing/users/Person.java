		...
		if(!guestObj.checkPersonIsGuest(personId))
		{
			...
			if(nokObj.checkIfNextOfKinPresent(studentId))
			{
				try
				{
					...
					String query3 = "UPDATE KIN_STUDENT SET first_name = ?, last_name = ?, street_name = ?," +
							" city_name = ?, zip_code = ?, phone_number = ?, gender = ?, DOB = ? WHERE student_id = ?";
					...
					preparedStatement3.setInt(9, studentId);
			...
			else
			{
				try
				{
					...
					String query3 = "INSERT INTO KIN_STUDENT (student_id,first_name,last_name,street_name,city_name,zip_code,phone_number,gender,DOB) VALUES(?,?,?,?,?,?,?,?,?)";
					preparedStatement3 = dbConnection3.prepareStatement(query3);
